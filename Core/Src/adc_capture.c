...
static uint16_t* const adcBuffer =
    (uint16_t*)ADC_CAPTURE_BUFFER_ADDRESS;

/* CPU-owned coherent copy; BDMA never writes this array. */
static uint16_t adcSnapshot[ADC_CAPTURE_BUFFER_SIZE];
...
HAL_StatusTypeDef ADC_Capture_Start(void) {
    ...
    if (HAL_ADC_Start_DMA(
            &hadc3,
            (uint32_t*)adcBuffer,
            ADC_CAPTURE_BUFFER_SIZE) != HAL_OK) {
        return HAL_ERROR;
    }

    /* Start TIM2 last so ADC3 and BDMA are ready for the first trigger. */
...
void HAL_ADC_ConvHalfCpltCallback(ADC_HandleTypeDef* hadc) {
    ...
    if (adcSnapshotState == ADC_SNAPSHOT_WAIT_HALF) {
        memcpy(
            &adcSnapshot[0],
            &adcBuffer[0],
            ADC_CAPTURE_HALF_BUFFER_SIZE * sizeof(uint16_t));
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc) {
    ...
    if (adcSnapshotState == ADC_SNAPSHOT_WAIT_FULL) {
        memcpy(
            &adcSnapshot[ADC_CAPTURE_HALF_BUFFER_SIZE],
            &adcBuffer[ADC_CAPTURE_HALF_BUFFER_SIZE],
            ADC_CAPTURE_HALF_BUFFER_SIZE * sizeof(uint16_t));
...
static void ADC_Capture_PrintSnapshot(void) {
    ...
    while (index < ADC_CAPTURE_BUFFER_SIZE) {
        ...
        for (uint32_t column = 0U;
             column < ADC_CAPTURE_PRINT_VALUES_PER_LINE &&
             index < ADC_CAPTURE_BUFFER_SIZE;
             column++, index++) {
            int written = snprintf(
                &line[length],
                sizeof(line) - length,
                "%u%s",
                (unsigned int)adcSnapshot[index],
                (column + 1U == ADC_CAPTURE_PRINT_VALUES_PER_LINE ||
                 index + 1U == ADC_CAPTURE_BUFFER_SIZE)
                    ? "\r\n"
                    : " ");

            if (written < 0 ||
                (size_t)written >= sizeof(line) - length) {
...
static void ADC_Capture_ReportRate(void) {
    ...
    uint32_t elapsed =
        (uint32_t)(now - adcRateTestStartTick);
    ...
    uint32_t fullCountDelta =
        currentFullCount - adcRateTestLastFullCount;

    uint64_t sampleCount =
        (uint64_t)fullCountDelta * ADC_CAPTURE_BUFFER_SIZE;

    uint32_t measuredSampleRate =
        (uint32_t)((sampleCount * 1000ULL) / elapsed);

    printf(
        "ADC rate: %lu samples/s, Half=%lu, Full=%lu, Error=%lu\r\n",
        (unsigned long)measuredSampleRate,
        (unsigned long)adcHalfCount,
        (unsigned long)adcFullCount,
        (unsigned long)adcErrorCount);
